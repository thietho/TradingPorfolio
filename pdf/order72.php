...
class PDF extends tFPDF
{
    ...
    private function showDetail($y)
    {
        ...
        $this->SetFont('Arial','B',8);
        $this->SetXY($this->papdingleft+23, $y);
        $this->Cell(15,4,"Price",0,1,"R");
        $this->SetXY($this->papdingleft+31, $y);
        $this->Cell(15,4,"Disc.",0,1,"R");
        $this->SetXY($this->papdingleft+40, $y);
        $this->Cell(12,4,"Qty",0,1,"R");
        $this->SetXY($this->papdingleft+40, $y);
        $this->Cell($this->papdingright - $this->papdingleft-40,4,"Total",0,1,"R");
        $this->SetFont('Arial','',8);
        $ycurent = $y;
        $datatable = array();
        if(isset($this->data['orderproducts']))
            foreach ($this->data['orderproducts'] as $i => $orderproduct)
            {
                ...
                {
                    $discount = (($price - $orderproduct["saleoff"])*100/$price)."%";
                }
                $row = array(
        ...
        foreach ($datatable as $i => $row)
        {
            ...
            $this->SetXY($this->papdingleft+23, $y+9 + ($i)*9);
            $this->Cell(15,4,$string->numberFormate($row['price']),0,0,"R");
            $this->SetXY($this->papdingleft+31, $y+9 + ($i)*9);
            $this->Cell(15,4,$row['discount'],0,0,"R");
        ...
        $ycurent += 4;
        $str = "Overdue";
        if($paid > $this->data['total'] )
            $str = "Change";
        $this->SetXY($this->papdingleft, $ycurent);
        $this->Cell($this->papdingleft + 35,4,$str,0,1,"R");
        $this->SetXY($this->papdingleft +40, $ycurent);
        $this->Cell($this->papdingright - $this->papdingleft-40,4,$this->data['setting']['currencyprefix'].$string->numberFormate( abs($this->data['total']-$paid)).$this->data['setting']['currencysubfix'],0,1,"R");
        return $ycurent;
    }
}
$datafile = $_GET['data'];
